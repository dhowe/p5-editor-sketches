...
function setup() {
  ...
  noStroke();
  camera(0, -150, 300);
  terrain = meshFromHeightmap(heightmap,50,50);
  print(terrain);
}
...
function draw() {
  ...
  // That results in excessively steep terrain for most use cases.
  rotateX(PI / 2);
  scale(1, 1, 50 / 255);
  model(terrain);
}
  ...
  return new p5.Geometry(detailX, detailY, function () {
    ...
    for (let j = 0; j <= detailY; j++) {
      for (let i = 0; i <= detailX; i++) {
        ...
        let vert = createVector(xoff + i * unitX, yoff + j * unitY, v);
